...
\begin{itemize}
	...
	\item \textbf{Page Fault Handler}, contains the kernel code to be executed during a page fault exception, Refer Section~\ref{sec:page fault}
...
It contains data pertaining to the current state of the process. The size of the PCB is \textbf{32 words}. Refer figure~\ref{fig:pcb}.\\
 ...
 The process identifier is a number from 0 to 31, which identifies the processes in memory. The PID of the current process can be calculated from the \texttt{PTBR} register.
	...
	\end{itemize}
	  ...
	  \label{sec:registers} 
\begin{itemize}
	\item \textbf{IP}: The word address of the currently executing instruction is stored in the \texttt{IP} (Instruction Pointer) register. The value of this instruction cannot be changed explicitly by any instruction. 
...
The Disk Free List is a data structure used for keeping track of unused blocks in the disk. The Disk Free List is stored in the page number 6 of the memory. 
...
The page 1 of the memory (\ref{chp:memoryorg}) is used as the scratchpad. Once the OS has booted up there is no need for the OS startup code. So this page can be reused as the scratchpad.
...
\subsubsection{Page Fault Exception Handler Routine}
